...
from mcp_client import McpAuth, McpClient, McpDenied, McpToolNames
...
class TestMcpAggregateNamespace:
    ...
    def test_one_session_lists_both_servers_and_routes_calls(
        ...
        both = (first_alias, second_alias)

        _ = client.poll_aggregate_tool_names(auth, both, until_listed=f"{first_alias}-echo")
        names = client.poll_aggregate_tool_names(auth, both, until_listed=f"{second_alias}-second_ping")
        expected = tuple(
            sorted(
        ...
        echoed = client.aggregate_call_tool(auth, both, f"{first_alias}-echo", {"text": payload})
        assert echoed.is_error is False, f"aggregate echo errored: {echoed.text[:300]}"
        assert echoed.text == payload

        pinged = client.aggregate_call_tool(auth, both, f"{second_alias}-second_ping", {})
        ...
        scoped = client.aggregate_list_tools_once(auth, (second_alias,))
        match scoped:
