...
from models import McpServerCreateBody, McpServerCredentials
...
class TestMcpUpstreamSharedKeyInjection:
    ...
    def test_stored_api_key_reaches_upstream_and_never_leaks(
        ...
        upstream_headers = client.stub_recorded_headers(alias, auth, f"{alias}-recorded_headers")
        assert upstream_headers.get("x-api-key") == MCP_STUB_UPSTREAM_API_KEY
        leaked = sorted(name for name, value in upstream_headers.items() if scoped_key in value)
