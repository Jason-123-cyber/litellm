...
from mcp_client import McpAuth, McpClient, McpToolNames, McpToolText
...
class TestMcpServerAccessControl:
    ...
    def test_key_without_grant_sees_no_tools_and_cannot_call(
        ...
        granted_auth = McpAuth(header_name="x-litellm-api-key", key=granted_key)
        denied_auth = McpAuth(header_name="x-litellm-api-key", key=denied_key)

        names = client.poll_tool_names(guarded_alias, granted_auth)
        ...
        result = client.call_tool(guarded_alias, granted_auth, f"{guarded_alias}-echo", {"text": payload})
        assert result.is_error is False, f"granted key's echo errored: {result.text[:300]}"
        assert result.text == payload

        _ = client.poll_tool_names(control_alias, denied_auth)

        denied_list = client.list_tools_once(guarded_alias, denied_auth)
        assert denied_list == McpToolNames(names=()), (
            f"key without the grant must be served an empty listing, got: {denied_list}"
        )

        denied_call = client.call_tool_once(guarded_alias, denied_auth, f"{guarded_alias}-echo", {"text": payload})
        match denied_call:
...
class TestMcpAllowedToolsFilter:
    ...
    def test_allowed_tools_filters_listing_and_blocks_excluded_call(
        ...
        blocked = client.call_tool_once(
            alias, auth, f"{alias}-slow_echo", {"text": payload, "marker": unique_marker(), "sleep_seconds": 0}
        )
        match blocked:
